...
function solve() {
    return function (animals) {
        ...
        _.each(groupedBySpecies, function (animalsGroup, key) {
            console.log(Array(key.length + 2).join('-'));
            console.log(key + ':');
            console.log(Array(key.length + 2).join('-'));

            var sortedAnimalsGroup = _.chain(animalsGroup)
                .sortBy('legsCount')
                .sortBy(animalsGroup, 'name')
                .value();

            _.each(sortedAnimalsGroup, function (animal) {
